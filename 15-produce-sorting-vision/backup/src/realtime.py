...
capture rate, something has to give, and the choice of what to drop is the whole design:
  ...
  * a Queue absorbs the burst, then never recovers -- latency grows without bound and
    you end up making decisions about a potato that left the frame seconds ago
...
(capture -> decision), not just model time.
    ...
    python realtime.py --source ../potato_img/raw/25.11.05/normal/SB
...
def run(source, weights, rate=None, conf=0.05, imgsz=1280, camera=False, loop=False,
    ...
    try:
        while True:
            ...
            if out_dir or display:
                vis = result.plot()
                cv2.putText(vis, f"e2e {e2e[-1]:5.1f} ms | drop {slot.dropped} | potato {n}",
    ...
    print(f"potato in          {hits}/{consumed} processed frames")
    ...
    if out_dir:
        ...
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["i", "t_capture", "has_potato", "n_boxes", "max_conf",
                        "infer_ms", "e2e_ms"])
            w.writerows(rows)
        print(f"\nwrote {csv_path}")
