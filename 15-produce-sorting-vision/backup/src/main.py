...
from backup.src.dataset import PotatoDataset
import glob
...
def main(steps, epochs, conf, limit):
    data_yaml = os.path.join(REPO, "datasets", "potato", "data.yaml")

    if "dataset" in steps:
        src = os.path.join(REPO, "potato_extern_dataset", "potato_downloads")
        root_paths = sorted(d for d in glob.glob(os.path.join(src, "*")) if os.path.isdir(d))
        builder = PotatoDataset(root_paths, os.path.join(REPO, "datasets", "potato"))
    ...
    if "tag" in steps:
        ...
        tag(weights=os.path.join(REPO, "runs", "detect", "potato", "weights", "best.pt"),
            source=os.path.join(REPO, "potato_img", "raw"),
