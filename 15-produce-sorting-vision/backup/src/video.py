...
own clip instead of splicing unrelated moments together.
    ...
    python video.py build  --frames ../potato_img/raw/25.11.05/normal/SB
...
def detect(weights, video_path, out_dir, conf=0.05, imgsz=1280, show_empty=True):
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        csv_writer.writerow(["frame", "t_sec", "has_potato", "n_boxes", "max_conf", "boxes_xyxy"])
        while True:
            ok, frame = cap.read()
            if not ok:
                break
            ...
            cv2.putText(vis, f"{stem}  f{n_frames:04d}  potato={n}",
    ...
    cap.release()
    writer.release()
    print(f"{stem}: {n_hit}/{n_frames} frames with potato, {total_boxes} boxes total")
    print(f"  -> {out_video}")
...
if __name__ == "__main__":
    ...
    if args.cmd == "build":
        ...
        for v in videos:
            ...
            frames += nf
        if len(videos) > 1:
            print(f"\ntotal: {hits}/{frames} frames with potato across {len(videos)} clips")
