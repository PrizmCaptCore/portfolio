"""Single-stage sorting pipeline: NanoDet decides normal/abnormal per box, tracks decide per potato.

The detector already answers "is this potato a reject", so there is no crop-and-classify
second stage any more. What survives from the two-stage version is the part that was never
about the classifier: a potato is seen over many frames, and the per-frame call is not the
decision.
...
# Stage 1 lives in detect_potato.py so it can be run and inspected on its own.
...
from ai_part.src.detect_potato import Detector
...
class Tracker:
    """Greedy IoU association. A conveyor moves potatoes on a smooth, near-linear path with
    ...
    box, so the same potato overlaps itself by a median IoU of 0.139 and this association
...
def main():
    ...
    print(f"  potatoes: {tally['normal']} pass / {tally['abnormal']} reject")
