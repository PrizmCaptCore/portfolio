"""Pull labelling frames out of raw belt footage: only frames with a item, ~3 per second.

Five minutes of 15 fps video is 4,500 frames, most of them either empty carrier or the same
item three pixels further along. Neither is worth a labeller's time. This walks each
video, keeps a frame only when the item detector sees one in it, and then only on a fixed
schedule of --fps per second -- so the output is N frames per second *of item-visible
time*, and stretches of empty carrier contribute nothing.

Presence is decided by the single-class item detector (runs/nanodet_item), not the
...
a dark cup carrier and could not tell a pocket from a item. The single-class model, trained
on 17.8k images for exactly the question "is there a item", was right on every frame
...
from detect_item import Detector, DEF_CFG, DEF_DET, REPO, VIDEO_EXT
...
def extract(det, path, out, args, writer):
    ...
    print(f"  {stem}  {i} frames -> {kept} kept  "
          f"({kept / max(i / fps, 1e-9):.2f}/s of video, target {args.fps}/s of item time)")
    return kept
...
def main():
    ...
    p.add_argument("--fps", type=float, default=3.0, help="frames kept per second of item time")
    p.add_argument("--det-thr", type=float, default=0.5,
                   help="presence floor. On this footage real items score 0.64-0.81 "
