...
trained separately: item (1 class, epoch~35 best) and binary normal/abnormal
...
Outputs under runs/multihead_test/:
    item-base.ckpt          original item weights, rewrapped   (harness baseline)
    binary-base.ckpt          original binary weights, rewrapped   (harness baseline)
    item-on-binaryBB.ckpt   item head transplanted onto binary backbone+fpn
    binary-on-itemBB.ckpt   binary head transplanted onto item backbone+fpn
...
ITEM_PTH = os.path.join(RUNS, "nanodet_item", "model_best", "nanodet_model_best.pth")
BINARY_PTH = os.path.join(RUNS, "nanodet_binary", "model_best", "nanodet_model_best.pth")
ITEM_CFG = os.path.join(REPO, "src", "config", "nanodet-plus-m-1.5x_416_item.yml")
...
def main():
    ...
    item, binary = load_sd(ITEM_PTH), load_sd(BINARY_PTH)
    ...
    diffs = [(item[k] - binary[k]).abs().mean().item()
             for k in item if k.startswith(SHARED) and item[k].dtype.is_floating_point]
    print(f"backbone+fpn mean |delta| between the two models: {sum(diffs) / len(diffs):.4f}")

    ckpts = {
        "item-base": item,
        "binary-base": binary,
        "item-on-binaryBB": splice(item, binary),
        "binary-on-itemBB": splice(binary, item),
    }
    ...
    for prefix, sd in (("head_item.", item), ("head_binary.", binary)):
...
def verify_and_time(ckpts):
    ...
    m_item_spliced = build(cfg_p, ckpts["item-on-binaryBB"])
    m_item_orig = build(cfg_p, ckpts["item-base"])
    ...
    two_models = bench(lambda: (m_binary(x), m_item_orig(x)))
    shared = bench(lambda: (lambda f: (m_binary.head(f), m_item_spliced.head(f)))(
