...
is wrong, and for a sorting line the how is the whole question: mAP treats a normal item
called abnormal and an abnormal one called normal as the same 0.5-point loss, when one
costs you a good item and the other ships a bad one to a customer.

So this buckets every image by what went wrong and writes it to its own folder:

    ok/            every box matched, right class
    false_reject/  a normal item called abnormal   <- you throw away good produce
    false_accept/  an abnormal item called normal  <- a defect reaches the customer
    ghost/         a box where there is no item
    missed/        a item with no box at all
...
from detect_item import Detector, REPO, IMAGE_EXT, VIDEO_EXT
...
TEST = os.path.join(REPO, "datasets", "item_binary_nanodet", "test")
...
def match(preds, gts, thr):
    ...
    Greedy rather than Hungarian: boxes here are whole items that barely overlap each
