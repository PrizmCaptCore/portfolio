"""Group each item in a clip into its own folder of crops, keyed off a reference line.

Frame-to-frame tracking does not work on this footage and the numbers say why: the belt
moves ~155 px per frame while a item box is ~219 px across, so the SAME item on two
consecutive frames overlaps itself by a median IoU of 0.139 -- under pipeline.Tracker's
iou_thr of 0.3. Association fails almost every frame, one item becomes several ids, and
...
detection on frame i with centre cx, the frame at which that item touches the line
x = LINE_X is

    phase = i + (LINE_X - cx) / dx

Every detection of the same item yields the same phase, whatever frame it came from.
Cluster (phase, cy) and each cluster is one physical item -- nothing is associated frame
to frame, so there is no chain to break. Item ids are then handed out in crossing order,
which is belt order. On the 10 SB clips (10 items each) this returns exactly 10 on 9 of
them, and 11 on one.
...
decides which frame is recorded as the crossing, i.e. which view is the canonical one.
    ...
    runs/tracks/SB_burst00/item_00/f00019.jpg   <- first item over the line
                                    /f00021.jpg
                          item_01/...
                          contact/item_00.jpg   <- one item, all its views, one sheet
                          items.csv
...
from ai_part.src.detect_item import Detector, DEF_CFG, DEF_DET, REPO, VIDEO_EXT
from ai_part.src.pipeline import crop          # one crop rule, shared with the live pipeline

# Edit these and run the file -- no CLI, same as detect_item.py.
SOURCE = os.path.join(REPO, "runs", "video", "SB")   # clip, or a folder of clips
OUT = os.path.join(REPO, "runs", "tracks")
CONFIG = DEF_CFG
WEIGHTS = DEF_DET
DET_THR = 0.20       # detect_item's default. Clustering tolerates a loose threshold far
...
P_TOL = 0.7          # phase tolerance, in frames. Two detections closer than this are the
                     # same item. Swept on the SB clips: 0.5-0.7 is the flat spot
Y_TOL = 150          # px. Separates the two lanes. Not tighter: a box clipped at the frame
                     ...
                     # item in two. 60-300 all hold the count; this is the middle of it
MIN_VIEWS = 3        # drop clusters with fewer crops -- those are flicker, not items
LINE_X = None        # crossing line in px; None = frame centre, where a item is whole
...
EDGE_MARGIN = 4      # px. A box touching the frame border is a partial item -- a lesion
                     # call on half a item is worse than no call, so those views are cut
CONTACT = True       # write the per-item contact sheet
...
def estimate_dx(dets, width):
    ...
    collapses each item's detections onto a single value.
    """
    if len(dets) < 20:
        return None
    ...
    for cand in np.arange(-width / 4, -width / 64, 0.5):
        ...
        # Sum only the small gaps: those are within-item spread. The large ones are the
        # gaps *between* items and should stay large, so leaving them out of the score
...
def assign(dets, dx, line_x, p_tol, y_tol, min_views):
    ...
    for phase, i, box in pts:
        # Nearest open cluster, not merely the first that fits -- with items tight on the
...
def contact_sheet(folder, cols=4, cell=160, limit=12):
    """One item, up to `limit` views evenly spaced over its life, tiled into one image."""
    ...
    # item turned, and consecutive frames are all one side.
...
def export_clip(detector, path, out_root, args):
    ...
    items, dropped = assign(dets, dx, line_x, args.p_tol, args.y_tol, args.min_views)
    ...
    want = collections.defaultdict(list)     # frame -> [(item_id, box), ...]
    for pid, items in enumerate(items):
        for i, box in items:
    ...
    seen = {pid: len(items) for pid, items in enumerate(items)}
    ...
    while True:
        ...
        for pid, box in want.get(i, []):
            # A box against the frame border is a item half out of shot; classifying that
            ...
            if c is None or min(c.shape[:2]) < args.min_px:
                small[pid] += 1
                continue
            d = os.path.join(out_root, f"item_{pid:02d}")
    ...
    if args.contact and kept:
        ...
        for pid in sorted(kept):
            sheet = contact_sheet(os.path.join(out_root, f"item_{pid:02d}"))
            if sheet is not None:
                cv2.imwrite(os.path.join(out_root, "contact", f"item_{pid:02d}.jpg"), sheet)

    with open(os.path.join(out_root, "items.csv"), "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["item_id", "n_crops", "n_views", "cross_frame", "first_frame",
    ...
    print(f"  {n_frames} frames, {len(dets)} boxes, belt {dx:+.0f} px/frame"
          f" -> {len(kept)} items, {sum(kept.values())} crops"
...
def main():
    ...
    for path in clips:
        ...
        print(f"\n=== {stem}")
        # Per clip, so item ids from different clips cannot collide in one folder.
        p, c = export_clip(detector, path, os.path.join(OUT, stem), args)
        n_pot += p
        n_crop += c

    print(f"\n{len(clips)} clips, {n_pot} items, {n_crop} crops -> {OUT}")
