"""Stage 1 only: run the trained NanoDet item detector and look at what it found.
...
Set the SOURCE/SAVE/... constants below and run `python detect_item.py`. This is a
...
behind flags -- typical uses:
    ...
    SOURCE = "../datasets/item/test/images";  SAVE = "../runs/detect_check"
...
To reproduce the mAP numbers in runs/nanodet_item/model_best/eval_results.txt, use
...
risk disagreeing with the numbers the model was selected on:
    ...
    cd <nanodet checkout> && python tools/test.py --task val \\
        --config <this repo>/ai_part/src/config/nanodet-plus-m-1.5x_416_item.yml \\
        --model <this repo>/ai_part/runs/nanodet_item/model_best/model_best.ckpt
"""
import io
import os
import time
...
DEF_CFG = os.path.join(REPO, "src", "config", "nanodet-plus-m-1.5x_416_item.yml")
DEF_DET = os.path.join(REPO, "runs", "nanodet_item", "model_best", "nanodet_model_best.pth")
...
class Detector:
    ...
    def __call__(self, frame, score_thr, with_class=False):
        ...
        # only ever ask "where are the items" (pipeline.py, export_tracks.py) keep the
...
def draw(frame, boxes):
    ...
    cv2.putText(frame, f"items: {len(boxes)}", (10, 26),
...
def run_images(detector, paths, args):
    ...
    for i, path in enumerate(paths):
        ...
        total += len(boxes)
        print(f"{os.path.basename(path):<40} {len(boxes):>3} items", flush=True)
