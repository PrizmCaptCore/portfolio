# 산업용 선별기 비전 파이프라인 — 카메라 검증부터 NanoDet 판정까지

> 컨베이어 컵에 실려 지나가는 제품을 GigE 카메라로 보고 정상/불량을 판정해 라인 컨트롤러에 신호를 주는
...
```
.
├── main.py / item.drawio          # 전체 루프 설계 메모 + 시스템 다이어그램
...
├── ai_part/
│   ├── src/config/*.yml             # NanoDet-Plus-m 1.5x 416 설정 (1-class item / 2-class normal·abnormal)
│   ├── src/detect_item.py         # Stage 1: Detector 래퍼 + 이미지/폴더/비디오/웹캠 시각 확인
...
│   └── runs/nanodet_item/model_best/eval_results.txt   # 학습 곡선 (가중치는 제외)
...
처음엔 "제품 검출 -> crop -> 불량 분류" 2-stage 였다. 검출기 자체가 normal/abnormal 을 답하도록 바꾸고
2-stage 를 버렸다. 남은 것은 분류기와 무관했던 부분, 즉 **한 제품은 여러 프레임에 걸쳐 보이고, 프레임별
판정은 결정이 아니라는 것**이다.
...
- `Detector` 는 `detect_item.py` 한 곳에만 있고 `pipeline.py` 가 import 한다. 추론 경로가 둘로 갈라져
  서로 어긋나는 일을 막는다.

1-class item 검출기(NanoDet-Plus-m 1.5x, 416) 학습 곡선 (`eval_results.txt`):
...
- 웹 사진으로 학습한 1-class 검출기는 **빈 컵 캐리어 휠 포켓**을 제품으로 잡는다(가로세로 ~2:1, conf 0.50~0.65).
  실제 제품은 ~1:1 이고 진짜 길쭉한 제품은 0.70 이상이라, `aspect >= 2.0 AND 재검출 conf < 0.65` 규칙으로
...
YOLO11 기반 초기 버전. 외부 Roboflow 데이터셋의 클래스명 난립('item', 'Items', '0')과 같은 원본 사진이
