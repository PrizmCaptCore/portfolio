"""Tag items in the in-house rig images -- no counting, no tracking.

Each image is classified as item / no-item by whether anything was detected,
and every detection is drawn as a box. Results are written to a CSV so the
has-item decision can be checked against the internal_IMG / background split.
"""
import os
import csv
import argparse
...
def tag(weights, source, out_dir, conf=0.25, imgsz=1280, save_images=True, limit=None):
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["image", "has_item", "n_boxes", "max_conf", "boxes_xyxy"])
    ...
    print(f"\n{n_item}/{len(paths)} images tagged as containing item")
...
if __name__ == "__main__":
    ...
    #item best weights: runs/detect/item/weights/best.pt
    parser.add_argument("--weights", default=os.path.join(repo, "src","runs", "detect", "item", "weights", "best.pt"))
    #item images: item_img/raw
    parser.add_argument("--source", default=os.path.join(repo, "item_img", "raw"))
