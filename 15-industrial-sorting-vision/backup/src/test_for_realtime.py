...
class LatestSlot:
    ...
    conveyor means acting on a item that has already gone past.
    """
    ...
    try:
        while True:
            ...
            if args.display or args.out:
                vis = r.plot()
                cv2.putText(vis, f"{(t1-t0)*1000:.1f} ms  item {n_box}",
...
def report(args, n, hits, elapsed, decode_ms, infer_ms, post_ms, render_ms, e2e_ms,
    ...
    print(f"\nitem in {hits}/{n} frames")

    if args.csv:
        os.makedirs(os.path.dirname(args.csv) or ".", exist_ok=True)
        with open(args.csv, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["i", "has_item", "n_boxes", "max_conf",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights", default=os.path.join(here, "runs", "detect", "item",
                                                          "weights", "best.pt"),
