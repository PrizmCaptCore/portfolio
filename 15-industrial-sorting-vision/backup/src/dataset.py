"""Merge the external Roboflow datasets into one single-class ("item") YOLO dataset.

The in-house rig images under item_img/ are deliberately NOT touched here -- they are
held out as the test set. This builds train/valid/test purely from item_extern_dataset/.

Two things the source datasets do that need handling:
  * class names vary wildly -- 'item', 'Items', 'Item', and literal '0'
...
class ItemDataset():
    ...
    def item_ids(self, root_path):
        """Class indices in this dataset that count as 'item'."""
        yaml_path = glob.glob(os.path.join(root_path, "*.yaml"))[0]
        with open(yaml_path, "r") as f:
            classes = yaml.safe_load(f)["names"]
        if isinstance(classes, dict):                       # {0: 'item', ...} form
            classes = [classes[k] for k in sorted(classes)]

        # Single-class datasets sometimes name the class '0'. Whatever it is called,
        # if there is only one class in a item dataset it is the item.
        if len(classes) == 1:
            return {0}
    ...
    def read_boxes(self, label_path, item_ids):
        """Return YOLO '0 cx cy w h' lines for the item objects in this label file."""
        lines = []
        with open(label_path, "r") as f:
            for line in f:
                ...
                if not parts:
                    continue
                if int(float(parts[0])) not in item_ids:
                    continue
    ...
    def collect(self):
        ...
        for root_path in self.root_paths:
            item_ids = self.item_ids(root_path)
            ...
            for mode in MODES:
                for label_path in glob.glob(os.path.join(root_path, mode, "labels", "*.txt")):
                    ...
                    if image_path is None:
                        ...
                        continue
                    boxes = self.read_boxes(label_path, item_ids)
                    ...
                    kept += 1
            print(f"  {name}: item class ids {sorted(item_ids)} -> {kept} images")
        return records
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src", default=os.path.join(repo, "item_extern_dataset", "item_downloads"))
    parser.add_argument("--out", default=os.path.join(repo, "datasets", "item"))
    parser.add_argument("--no-defective", action="store_true",
                        help="treat damaged/diseased/sprouted items as non-item")
    parser.add_argument("--no-negatives", action="store_true",
                        help="drop images with no item instead of keeping them as background")
    ...
    dataset = ItemDataset(root_paths, args.out,
