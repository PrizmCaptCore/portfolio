...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    let microphone_device = match preferred_mic_name {
        Some(pref_name) => {
            ...
            match parse_audio_device(&pref_name) {
                ...
                Err(e) => {
                    warn!("Preferred microphone '{}' not available: {}", pref_name, e);
    ...
    let system_device = match preferred_system_name {
        Some(pref_name) => {
            ...
            match parse_audio_device(&pref_name) {
                ...
                Err(e) => {
                    warn!(
                        "Preferred system audio '{}' not available: {}",
                        pref_name, e
                    );
                    ...
                    match default_output_device() {
                        ...
                        Err(default_err) => {
                            warn!(
                                "No system audio available (preferred and default both failed): {}",
                                default_err
                            );
        ...
        .await
    {
        ...
        Err(e) => {
            let user_message = format!("Failed to start recording: {}", e);
            error!("{}", user_message);
            let _ = app.emit(
                "recording-error",
                serde_json::json!({
    ...
    // Auto-stop after extended silence (warning at 2min, stop at 3min)
    transcription::silence::start_silence_monitor(app.clone());
        ...
        Err(e) => {
            ...
            let _ = app.emit(
                ...
                serde_json::json!({
                    "error": user_message,
                    "userMessage": user_message,
                    "stage": "start_recording_with_devices"
                }),
            );
            return Err(user_message);
        }
...
pub async fn stop_recording<R: Runtime>(
    ...
    // Cancel the silence monitor first so an auto-stop firing right now
    // can't recursively re-enter stop_recording mid-shutdown.
    super::transcription::silence::abort_silence_monitor();
    ...
    if let Some(task_handle) = transcription_task {
        ...
        .await
        {
            ...
            Err(_) => {
                warn!("Transcription timeout (10 minutes) reached, continuing shutdown to prevent indefinite hang");
    ...
    // Close the streaming STT WebSocket so Modal's scaledown timer can start.
    // Safe to do here because the transcription task above already drained
    // every queued chunk; no further .transcribe() calls will happen.
    super::transcription::close_nemo_provider().await;
    ...
    .await
    {
        ...
        Err(_) => {
            warn!("Transcript config timeout (30s), continuing shutdown");
            None
        }
    };

    // Cloud-based transcription: no local model to unload
    info!("Cloud-based transcription provider, no local model to unload");

    // Step 3.5: analytics removal deferred; keep shutdown path simple for now
    ...
    let (meeting_folder, meeting_name) = if let Some(mut manager) = manager_for_cleanup {
        ...
        .await
        {
            ...
            Ok(Err(e)) => {
                warn!(
                    "Error during recording cleanup (transcripts preserved): {}",
                    e
                );
                // Don't fail shutdown - transcripts are already preserved
            }
            Err(_) => {
                warn!("File I/O timeout (5 minutes) reached during save, continuing shutdown");
                // Don't fail shutdown - transcripts are already preserved
            }
        }
        ...
        info!("No recording manager available for cleanup");
        (None, None)
    };
    ...
    info!("Skipping database save in Rust - frontend will save after all transcripts received");
