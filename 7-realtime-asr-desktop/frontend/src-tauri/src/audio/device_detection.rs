...
impl InputDeviceKind {
    ...
    pub fn detect(device_name: &str, buffer_size: u32, sample_rate: u32) -> Self {
        ...
        warn!(" Could not determine device type for '{}', using conservative (Bluetooth-like) settings", device_name);
    ...
    fn detect_by_name(device_name: &str) -> Option<Self> {
        ...
        for pattern in TIER3_BLUETOOTH_PATTERNS {
            if name_lower.contains(pattern) {
                warn!(
                    " Tier 3 Bluetooth pattern matched: '{}' (pattern: '{}') - lower confidence",
                    device_name, pattern
                );
                return Some(InputDeviceKind::Bluetooth);
            }
    ...
    fn detect_by_buffer_size(buffer_size: u32, sample_rate: u32) -> Option<Self> {
        ...
        if base_latency_ms > 50.0 {
            warn!(
                " High buffer latency detected: {:.2}ms (buffer_size={}, sample_rate={})",
        ...
        debug!(
            " Ambiguous buffer latency: {:.2}ms - cannot determine device type from buffer size",
            base_latency_ms
        );
        None
    }
