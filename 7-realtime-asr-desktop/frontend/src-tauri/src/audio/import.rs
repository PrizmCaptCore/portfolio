...
use crate::audio::transcription::OpenAIProvider;
...
pub async fn start_import<R: Runtime>(
    ...
    super::common::unload_engine_after_batch(false).await;
...
async fn run_import<R: Runtime>(
    ...
    let openai_provider = if total_segments > 0 {
        Some(init_openai_provider(&app, model.as_deref()).await?)
    } else {
        None
    };
...
/// Initialize an OpenAI transcription provider from app config
async fn init_openai_provider<R: Runtime>(
    app: &AppHandle<R>,
    model: Option<&str>,
) -> Result<OpenAIProvider> {
    let config = crate::api::api::api_get_transcript_config(app.clone(), app.clone().state(), None)
        .await
        .map_err(|e| anyhow!("Failed to get transcript config: {}", e))?;

    let api_key = config.and_then(|c| c.api_key).ok_or_else(|| {
        anyhow!("OpenAI API key not configured. Please set your API key in settings.")
    })?;

    let model_name = model.unwrap_or("gpt-4o-transcribe").to_string();
    Ok(OpenAIProvider::new(api_key, model_name))
}
...
mod tests {
    ...
    fn optional_sample_path(file_name: &str) -> Option<std::path::PathBuf> {
        if let Ok(path) = std::env::var("TEST_AUDIO_PATH") {
        ...
        let candidates = [
            std::path::PathBuf::from(format!("tests/fixtures/{}", file_name)),
            std::path::PathBuf::from(format!("src-tauri/tests/fixtures/{}", file_name)),
        ];
    ...
    fn test_extract_duration_from_metadata_wav() {
        if let Some(test_path) = optional_sample_path("jfk.wav") {
    ...
    fn test_extract_duration_from_metadata_mp3() {
        if let Some(test_path) = optional_sample_path("jfk.mp3") {
            let result = extract_duration_from_metadata(&test_path);
