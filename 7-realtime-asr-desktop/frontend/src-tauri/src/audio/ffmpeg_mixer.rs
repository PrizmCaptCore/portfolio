        ...
        if let Some(last_time) = self.last_chunk_time {
            ...
            if gap_duration > expected_duration.mul_f32(2.0) {
                ...
                if self.device_kind.is_bluetooth() {
                    debug!(
                        " Gap detected in '{}': {:.1}ms (expected ~{:.1}ms)",
                        self.device_name,
                    ...
                    warn!(
                        " Unexpected gap in wired device '{}': {:.1}ms",
                        self.device_name,
                        gap_duration.as_secs_f64() * 1000.0
                    );
                }
...
impl FFmpegAudioMixer {
    ...
    pub fn new(
        ...
        info!(" Creating FFmpeg Adaptive Audio Mixer");
        info!(
            "   Microphone: '{}' ({:?})",
    ...
    fn log_stats(&self) {
        ...
        info!(" Mixer Statistics (after {} windows):", self.windows_mixed);
        info!(
