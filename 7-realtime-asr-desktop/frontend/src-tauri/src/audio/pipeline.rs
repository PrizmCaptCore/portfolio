...
/// Prevents feeding dead air to the recognizer (causes "yeah yeah" hallucination).
/// Note: energy = mean(x²), so RMS = sqrt(energy).
/// 0.005 → RMS ~0.071 (too aggressive, blocks quiet remote speakers)
/// 0.0001 → RMS ~0.01 (passes quiet speech, server-side VAD filters the rest)
const SILENCE_ENERGY_THRESHOLD: f32 = 0.0001;
...
impl AudioMixerRingBuffer {
    ...
    fn add_samples(&mut self, device_type: DeviceType, samples: Vec<f32>) {
        ...
        if self.mic_buffer.len() > self.max_buffer_size {
            warn!(
                " Microphone buffer overflow: {} > {} samples, dropping oldest {} samples",
                self.mic_buffer.len(),
                self.max_buffer_size,
...
impl AudioCapture {
    pub fn new(
        ...
        if needs_resampling {
            warn!(" SAMPLE RATE MISMATCH DETECTED ");
            warn!(
        ...
        let (noise_suppressor, high_pass_filter, normalizer) = if matches!(
            ...
            let ns = if super::ffmpeg_mixer::RNNOISE_APPLY_ENABLED {
                ...
                info!("ℹ RNNoise noise suppression DISABLED for microphone '{}' (flag: RNNOISE_APPLY_ENABLED=false)", device.name);
            ...
            let norm = match LoudnessNormalizer::new(1, TARGET_SAMPLE_RATE) {
                ...
                Err(e) => {
                    warn!(
                        " Failed to create normalizer for microphone: {}, normalization disabled",
                        e
                    );
                    None
                }
            ...
            info!(
                "ℹ System audio '{}' captured raw (no enhancement)",
                device.name
            );
            (None, None, None)
        };
        ...
        let resampler = if needs_resampling {
            ...
            match SincFixedIn::<f32>::new(
                ...
                Err(e) => {
                    warn!(
                        " Failed to create persistent resampler: {}, will use fallback",
                        e
                    );
                    None
                }
    ...
    pub fn process_audio_data(&self, data: &[f32]) {
                    ...
                    if let Some(ref mut resampler) = *resampler_lock {
                        ...
                        while buffer_lock.len() >= self.resampler_chunk_size {
                            ...
                            match resampler.process(&waves_in, None) {
                                ...
                                Err(e) => {
                                    warn!(" Persistent resampler processing failed: {}", e);
                ...
                if let Ok(mut ns_lock) = self.noise_suppressor.lock() {
                    if let Some(ref mut suppressor) = *ns_lock {
                        ...
                        if chunk_id % 100 == 0 {
                            ...
                            if buffered > 1000 {
                                warn!(" RNNoise accumulating samples: {} buffered (potential latency issue!)",
                                      buffered);
                            }
                            ...
                            if length_delta > 50 {
                                warn!(
                                    " RNNoise length mismatch: input={} output={} (delta={})",
        ...
        // DIAGNOSTIC: Log audio levels every 100 chunks (~5s at 48kHz/1024 chunks)
        if chunk_id % 100 == 0 && !mono_data.is_empty() {
            ...
            if matches!(self.device_type, DeviceType::Microphone) {
                debug!(
                    " [Mic] Chunk {} - RMS={:.6}, Peak={:.6}",
                    chunk_id, raw_rms, raw_peak
                );
                if raw_rms == 0.0 && raw_peak == 0.0 {
                    warn!(" Microphone producing ZERO audio - check permissions or hardware!");
                }
            } else {
                info!(
                    " [Sys] Chunk {} - RMS={:.6}, Peak={:.6}",
                    chunk_id, raw_rms, raw_peak
                );
                if raw_rms == 0.0 && raw_peak == 0.0 {
                    warn!(" System audio producing ZERO audio - check Screen Recording / Audio Capture permissions in System Preferences!");
                }
            }
        }
...
/// Session-based audio processing pipeline.
///
/// Instead of VAD-based segmentation, this pipeline continuously accumulates
/// resampled audio and emits it to the STT engine at fixed intervals (~800ms).
/// A simple energy gate prevents feeding silence (which causes Whisper hallucination).
///
/// The STT engine (WhisperProvider) maintains a persistent session state, so
/// each call carries forward context from previous transcriptions — giving the
/// effect of continuous, context-aware listening.
...
impl AudioPipeline {
    pub fn new(
        ...
        info!(" AudioPipeline initializing (session mode, Silero VAD):");
        info!(
            "   Mic: '{}' ({:?}) - Buffer: {:?}",
            mic_device_name,
            mic_device_kind,
        ...
        // VAD runs server-side in the gateway — pipeline just sends all audio.
        info!("Audio pipeline initialized (VAD handled server-side)");
        ...
        Self {
            ...
            latest_chunk_timestamp: 0.0,
        }
    }

    /// Run the session-based audio processing pipeline
    pub async fn run(mut self) -> Result<()> {
        info!("Streaming audio pipeline started (gateway handles VAD + ASR)");

        loop {
            match tokio::time::timeout(std::time::Duration::from_millis(50), self.receiver.recv())
                .await
            {
                Ok(Some(chunk)) => {
                    // Flush signal — just log and continue (no buffer to flush)
                    ...
                    self.latest_chunk_timestamp = chunk.timestamp;

                    // Metrics batching (unchanged)
                    ...
                    // STEP 2: Extract mixed windows and accumulate for STT
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // Track per-channel energy for speaker identification
                            let mic_energy: f64 = mic_window
                                .iter()
                                .map(|x| (*x as f64) * (*x as f64))
                                .sum::<f64>()
                                / mic_window.len().max(1) as f64;
                            let sys_energy: f64 = sys_window
                                .iter()
                                .map(|x| (*x as f64) * (*x as f64))
                                .sum::<f64>()
                                / sys_window.len().max(1) as f64;

                            // Clamp to [-1.0, 1.0]
                            let resampled_clamped: Vec<f32> =
                                resampled.iter().map(|x| x.clamp(-1.0, 1.0)).collect();

                            // Send all audio to worker — VAD runs server-side in the gateway.
                            // Energy gate only filters dead silence to reduce unnecessary processing.
                            let energy: f32 = resampled_clamped.iter().map(|x| x * x).sum::<f32>()
                                / resampled_clamped.len().max(1) as f32;

                            if energy > SILENCE_ENERGY_THRESHOLD
                                && !resampled_clamped.is_empty()
                                && !self.transcription_sender.is_closed()
                            {
                                // Stable chunk_id: don't increment per frame.
                                // All streaming chunks share the same ID so the frontend
                                // updates one line. Worker changes ID on endpoint (final).
                                let speaker = if mic_energy > sys_energy * 1.5 {
                                    DeviceType::Microphone
                                } else {
                                    DeviceType::System
                                };

                                let stt_chunk = AudioChunk {
                                    data: resampled_clamped,
                                    sample_rate: 16000,
                                    timestamp: chunk.timestamp,
                                    chunk_id: self.chunk_id_counter, // stable per session
                                    device_type: speaker,
                                    is_partial: true,
                                };
                                let _ = self.transcription_sender.send(stt_chunk);
                            }
                ...
                Err(_) => {
                    // Timeout — no action needed, gateway handles state internally
                    continue;
                }
            }
        }

        info!("Audio pipeline ended (processed {} chunks)", self.processed_chunks);
        Ok(())
    }
...
impl AudioPipelineManager {
    ...
    pub fn start(
        ...
        info!(" Starting pipeline with device info:");
        info!(
            "   Microphone: '{}' ({:?})",
        ...
        info!("Audio pipeline manager: constructing pipeline");
        ...
        info!("Audio pipeline manager: pipeline constructed successfully");
...
mod tests {
    ...
    #[test]
    fn energy_threshold_rejects_silence() {
        let silence = vec![0.0001_f32; 16000]; // ~1s of near-silence
        let energy: f32 = silence.iter().map(|x| x * x).sum::<f32>() / silence.len() as f32;
        assert!(energy < SILENCE_ENERGY_THRESHOLD);
    }

    #[test]
    fn energy_threshold_accepts_speech() {
        let speech = vec![0.1_f32; 16000]; // ~1s of moderate speech
        let energy: f32 = speech.iter().map(|x| x * x).sum::<f32>() / speech.len() as f32;
        assert!(energy > SILENCE_ENERGY_THRESHOLD);
    }
}
