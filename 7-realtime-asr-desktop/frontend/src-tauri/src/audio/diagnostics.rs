...
fn check_for_issues(
    ...
    if detected_kind.is_bluetooth() && buffer_latency_ms < 50.0 {
        warn!(
            " POTENTIAL ISSUE: Bluetooth device has unusually low buffer latency ({:.2}ms)",
            buffer_latency_ms
        );
    ...
    if detected_kind.is_wired() && buffer_latency_ms > 50.0 {
        warn!(
            " POTENTIAL ISSUE: Wired device has unusually high buffer latency ({:.2}ms)",
            buffer_latency_ms
        );
    ...
    if sample_rate != 48000 && sample_rate != 0 {
        info!(
            "ℹ NOTE: Device uses non-standard sample rate ({} Hz)",
            sample_rate
        );
    ...
    if channels > 1 {
        info!(
            "ℹ NOTE: Device has {} channels (stereo/multi-channel)",
            channels
        );
...
pub fn log_buffer_health(
    ...
    if buffer_utilization > 80.0 {
        ...
        warn!(" BUFFER UNDERRUN: '{}' ({:?})", device_name, device_kind);
        warn!("   Dropped frames: {}", dropped_frames);
    }
}
...
pub fn log_performance_summary(
    ...
    info!(
        "  Buffer Overflows:    {} {}",
        buffer_overflows,
        if buffer_overflows == 0 { "" } else { "" }
    );
    info!("  Device Reconnects:   {}", device_reconnects);
    info!("╚═══════════════════════════════════════════════════════════╝");

    if buffer_overflows > 0 {
        warn!(" Buffer overflows detected! This may indicate:");
