...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\relay_assistant
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("relay_assistant")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("relay_assistant")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/relay_assistant
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("relay_assistant")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/relay_assistant
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("relay_assistant")
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if backend_enum == AudioCaptureBackend::CoreAudio {
            ...
            if !check_screen_recording_permission() {
                warn!("  Audio Capture permission may not be granted");
