...
impl RecordingManager {
    ...
    pub async fn start_recording(
        ...
        if let Err(e) = self.state.start_recording() {
            error!(
                "Recording manager failed before pipeline start: state.start_recording: {}",
                e
            );
            return Err(e);
        }
        ...
        if let Err(e) = self.pipeline_manager.start(
            ...
            error!("Recording manager failed during pipeline startup: {}", e);
        ...
        {
            error!(
                "Recording manager failed during audio stream startup: {}",
                e
            );

            if let Err(stop_err) = self.pipeline_manager.stop().await {
                error!(
                    "Recording manager cleanup failed after stream startup error: {}",
                    stop_err
                );
            }
            self.state.stop_recording();
    ...
    pub async fn start_recording_with_defaults_and_auto_save(
        ...
        #[cfg(target_os = "macos")]
        {
            info!(" [macOS] Starting recording with smart device selection (Bluetooth override enabled)");
...
impl Drop for RecordingManager {
    fn drop(&mut self) {
        if self.state.is_recording() {
            warn!(
                "RecordingManager dropped while recording was still active; forcing state cleanup"
            );
        }
