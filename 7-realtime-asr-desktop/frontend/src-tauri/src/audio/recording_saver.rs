...
/// Minimum interval between transcript disk writes (debounce)
const TRANSCRIPT_FLUSH_INTERVAL_MS: u64 = 2000;
...
pub struct RecordingSaver {
    ...
    transcript_dirty: Arc<AtomicBool>,
...
impl RecordingSaver {
    pub fn new() -> Self {
        Self {
            ...
            transcript_dirty: Arc::new(AtomicBool::new(false)),
            chunk_receiver: None,
    ...
    pub fn add_transcript_segment(&self, segment: TranscriptSegment) {
        ...
        // Mark dirty — background flush task writes to disk periodically
        self.transcript_dirty.store(true, Ordering::Release);
    }
    ...
    pub fn start_accumulation(&mut self, auto_save: bool) -> mpsc::UnboundedSender<AudioChunk> {
        ...
        // Start background transcript flush task (debounced disk writes)
        if let Some(folder) = self.meeting_folder.clone() {
            let dirty = self.transcript_dirty.clone();
            let segments = self.transcript_segments.clone();
            let is_saving = self.is_saving.clone();

            self.flush_task_handle = Some(tokio::spawn(async move {
                info!(
                    "Transcript flush task started (interval: {}ms)",
                ...
                loop {
                    ...
                    let active = is_saving.lock().map(|g| *g).unwrap_or(false);
                    if !active {
                        // Final flush before exit
                        if dirty.swap(false, Ordering::AcqRel) {
                            let _ = write_segments_to_disk_static(&segments, &folder);
                        }
                        break;
                    }

                    if dirty.swap(false, Ordering::AcqRel) {
                        if let Err(e) = write_segments_to_disk_static(&segments, &folder) {
                            warn!("Flush task: failed to write transcripts: {}", e);
                        }
                    }
                }
                info!("Transcript flush task ended");
            }));
        }
    ...
    fn write_transcripts_json(&self, folder: &PathBuf) -> Result<()> {
        write_segments_to_disk_static(&self.transcript_segments, folder)
    }
    ...
    pub async fn stop_and_save<R: Runtime>(
        ...
        // Stop accumulation (also signals flush task to exit on next tick)
        if let Ok(mut is_saving) = self.is_saving.lock() {
            *is_saving = false;
        }

        // Wait for flush task to do its final write and exit
        ...
        if !should_save_audio {
            info!("  No audio saver initialized (auto-save was disabled) - skipping audio finalization");
...
/// Shared disk-write logic usable from both &self methods and the background flush task.
fn write_segments_to_disk_static(
    segments_arc: &Arc<Mutex<Vec<TranscriptSegment>>>,
    folder: &PathBuf,
) -> Result<()> {
    let segments_clone = if let Ok(segments) = segments_arc.lock() {
        segments.clone()
    } else {
