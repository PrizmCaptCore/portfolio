...
impl ContinuousVadProcessor {
    ...
    /// Returns true when VAD currently considers the stream to be inside an active speech turn.
    pub fn is_in_speech(&self) -> bool {
        self.in_speech && !self.current_speech.is_empty()
    }

    /// Snapshot the in-progress speech turn for low-latency live preview updates.
    /// When `max_samples` is set, only the most recent tail of the turn is returned.
    pub fn snapshot_current_speech(&self, max_samples: Option<usize>) -> Option<SpeechSegment> {
        if !self.is_in_speech() {
            return None;
        }

        let total_samples = self.current_speech.len();
        let start_offset = max_samples
            .map(|limit| total_samples.saturating_sub(limit))
            .unwrap_or(0);
        ...
        let start_sample = self.speech_start_sample + start_offset;
