...
/// If a migration file was edited after it was already applied, sqlx refuses to
/// run with "previously applied but has been modified".  We don't want to
/// re-apply the migration (the schema change is already in the DB), we just
/// need to update the stored checksum so sqlx agrees with the file on disk.
async fn fix_dirty_migration_checksums(pool: &SqlitePool) {
    let migrator = sqlx::migrate!("./migrations");

    // Collect current file checksums: version → bytes
    let file_checksums: std::collections::HashMap<i64, Vec<u8>> = migrator
        .migrations
        .iter()
        .map(|m| (m.version, m.checksum.to_vec()))
        .collect();

    // Fetch what's stored in the DB
    let rows: Vec<(i64, Vec<u8>)> =
        ...
        {
            Ok(r) => r,
            Err(_) => return, // table doesn't exist yet — nothing to fix
        };

    for (version, stored) in rows {
        if let Some(expected) = file_checksums.get(&version) {
            if *expected != stored {
                log::warn!(
                    "Migration {} checksum mismatch (file modified after apply). \
                     Updating stored checksum to match current file.",
                    version
                );
                let _ = sqlx::query(
            ...
            if msg.contains("previously applied but has been modified") {
                log::warn!(
                    "Migration checksum mismatch detected ({}). \
                     Fixing stored checksums and retrying...",
                    msg
                );
                fix_dirty_migration_checksums(&pool).await;
