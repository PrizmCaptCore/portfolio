...
pub async fn initialize_fresh_database(app: AppHandle) -> Result<(), String> {
    ...
    // Default Summary Model: external provider placeholder.
    // Users configure the actual endpoint and model in settings.
    ...
    // Default transcription: CNN STT (real-time friendly, light on resources)
    if let Err(e) =
