...
/// Live transcript assistant: intent summary (assistant language) and English reply ideas (summary LLM).
#[tauri::command]
pub async fn api_live_assist_transcript_segment<R: Runtime>(
    app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
    text: String,
    target_language_name: String,
    prior_transcript_context: Option<String>,
    _auth_token: Option<String>,
) -> Result<LiveAssistResult, String> {
    let pool = state.db_manager.pool();
    let prior = prior_transcript_context.and_then(|s| {
    ...
    SummaryService::live_assist_segment(&app, pool, &text, prior.as_deref(), &target_language_name)
        .await
}

/// Drag-suggest entry point. Mirrors `api_translate_drag`'s procedure but
/// uses the summary-specific endpoints — `/ai/health/` does not warm the
/// summary container (it only kicks STT + translate), so we kick
/// `/ai/summary/warmup/` and poll `/ai/summary/ready/` instead. After
/// Modal summary confirms ready, we delegate to the existing
/// `live_assist_segment` flow for the real LLM call.
///
/// Separate from `api_live_assist_transcript_segment` because that one
/// runs continuously during recording (via live assist triggers) where
/// pre-flight has already happened; this wrapper is for the user-initiated
/// drag-suggest click where the container may have scaled down.
#[tauri::command]
pub async fn api_suggest_drag<R: Runtime>(
    app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
    text: String,
    target_language_name: String,
    ...
    let warmup_url = format!("{}/summary/warmup/", base);
    let ready_url = format!("{}/summary/ready/", base);

    // Re-read the JWT from disk on each HTTP call; this function runs for up
    // to ~4.5 minutes across warmup + ready polling, long enough to cross a
    // token-expiry boundary. See `api_translate_drag` for the same pattern.
    ...
    // Step 1: kick /ai/summary/warmup/. Gateway returns 200 immediately and
    // pings Modal summary /health in a background thread.
    log_info!("api_suggest_drag: kicking summary warmup {}", warmup_url);
    ...
    loop {
        ...
        if std::time::Instant::now() >= warmup_deadline {
            return Err("gateway /summary/warmup/ did not respond within 30s".to_string());
        }
        tokio::time::sleep(std::time::Duration::from_secs(5)).await;
    }

    // Step 2: poll /ai/summary/ready/ until 200.
    log_info!("api_suggest_drag: polling {} until ready", ready_url);
    ...
    loop {
            ...
            .await
        {
            Ok(r) if r.status().is_success() => {
                log_info!("api_suggest_drag: summary ready after {} polls", iter);
                break;
            }
            Ok(r) => {
                log_info!(
                    "api_suggest_drag: poll {} got {}, retrying",
                    iter,
                    r.status()
                );
            }
            Err(e) => {
                log_info!("api_suggest_drag: poll {} err: {}", iter, e);
            }
        }
        if std::time::Instant::now() >= ready_deadline {
            return Err("Modal summary did not become ready within 240s".to_string());
        }
        tokio::time::sleep(std::time::Duration::from_secs(5)).await;
    }

    // Step 3: delegate to the existing live_assist_segment flow.
    let pool = state.db_manager.pool();
    let prior = prior_transcript_context.and_then(|s| {
        let t = s.trim().to_string();
        if t.is_empty() {
