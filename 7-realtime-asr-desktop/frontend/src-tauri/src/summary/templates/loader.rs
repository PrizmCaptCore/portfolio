...
/// - macOS: ~/Library/Application Support/Relay Assistant/templates/
/// - Windows: %APPDATA%\Relay Assistant\templates\
/// - Linux: ~/.config/Relay Assistant/templates/
fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push("Relay Assistant");
    path.push("templates");
    Some(path)
}
