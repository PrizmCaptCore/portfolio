    ...
    fn default() -> Self {
        Self {
            ...
            model_status: ModelStatus {
                cnn_stt: "not_applicable".to_string(),
                summary: "not_applicable".to_string(),
            },
...
pub async fn complete_onboarding<R: Runtime>(
    ...
    info!("Completing onboarding with external summary provider placeholder");

    // Step 1: Save model configuration to SQLite database FIRST
    let pool = state.db_manager.pool();

    // Onboarding no longer locks the app into the removed built-in sidecar.
    // Users can configure their external summary provider after setup.
    if let Err(e) =
        SettingsRepository::save_model_config(pool, "relay-ai", "", "large-v3").await
    {
        error!("Failed to save default summary model config: {}", e);
        return Err(format!(
            "Failed to save default summary model config: {}",
            e
        ));
    }
    info!("Saved default summary model config for external setup");

    // Transcription default matches onboarding download (CNN STT — real-time, efficient)
    ...
    info!(
        "Saved transcription model config: provider=cnn-stt, model={}",
        crate::config::DEFAULT_CNN_STT_MODEL
    );
    ...
    status.model_status.cnn_stt = "not_applicable".to_string();
