...
pub fn create_tray<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    let mut builder = TrayIconBuilder::with_id("main-tray")
        .menu(&menu)
        .tooltip("Relay Assistant")
        .on_menu_event(|app, event| handle_menu_event(app, event.id.as_ref()));

    if let Some(icon) = app.default_window_icon() {
        builder = builder.icon(icon.clone());
    } else {
        log::warn!("System tray icon is unavailable; creating tray without a custom icon");
    }
...
/// - CNN STT transcription model is ready (downloaded)
async fn check_can_record<R: Runtime>(app: &AppHandle<R>) -> bool {
    ...
    // Cloud-based transcription is always available
    true
}
