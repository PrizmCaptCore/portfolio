...
export function ImportAudioDialog({
  ...
  const handleImportComplete = useCallback((result: ImportResult) => {
    toast.success(`가져오기 완료! 세그먼트 ${result.segments_count}개를 생성했습니다.`);
  ...
  const selectedModel = useMemo((): { key: string; label: string; provider: string; name: string; displayName: string; size_mb: number } | undefined => {
  ...
  const isCnnSttModel = selectedModel?.provider === 'cnn-stt';
  ...
  const handleStartImport = async () => {
    ...
    await startImport(
      ...
      isCnnSttModel ? null : selectedLang === 'auto' ? null : selectedLang,
