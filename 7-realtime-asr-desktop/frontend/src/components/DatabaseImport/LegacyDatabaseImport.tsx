...
export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage('선택한 위치에서 데이터베이스를 찾지 못했습니다. 앱 폴더, 백엔드 폴더 또는 데이터베이스 파일을 직접 선택해 주세요.');
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success('데이터베이스를 가져왔습니다! 새로 고치는 중…');
      ...
      setErrorMessage(String(error));
      setImportState('error');
      toast.error(`가져오기에 실패했습니다: ${error}`);
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  const handleStartFresh = async () => {
    try {
      ...
      setImportState('success');
      toast.success('데이터베이스를 초기화했습니다! 앱을 시작합니다…');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">Relay Assistant에 오신 걸 환영합니다!</DialogTitle>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              이전에 사용하던 앱 폴더, 백엔드 디렉터리 또는 데이터베이스 파일을 선택해 주세요:
            </p>
            ...
            >
              {importState === 'selecting' || importState === 'detecting' ? (
                <>
                  ...
                  <span>{importState === 'selecting' ? '선택 중…' : '데이터베이스 감지 중…'}</span>
                </>
              ) : (
                <>
                  <FolderOpen className="h-5 w-5" />
                  <span>데이터베이스 찾아보기</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">데이터베이스를 찾았습니다!</p>
