...
interface SidebarContextType {
  ...
  updateSearchQuery: (query: string) => Promise<void>;
...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      setServerAddress('local://app');
      setTranscriptServerAddress('local://transcript');
    };
    fetchSettings();
  }, []);
  ...
  // Update current meeting when on the recording page (which used to be `/`).
  // The "+ New Call" placeholder is the active "row" while the user is on the
  // recording surface and hasn't yet saved a meeting.
  useEffect(() => {
    if (pathname === '/record') {
  ...
  const handleRecordingToggle = () => {
    if (!isRecording) {
      // Check if already on the recording page
      if (pathname === '/record') {
        // Already on /record — trigger recording directly via custom event
        console.log('Triggering recording from sidebar (already on /record)');
        window.dispatchEvent(new CustomEvent('start-recording-from-sidebar'));
      } else {
        // Not on /record — navigate and use auto-start mechanism
        console.log('Navigating to /record with auto-start flag');
      ...
      // If stop is requested outside home, navigate home and auto-stop there.
      if (pathname !== '/') {
    ...
    // The actual recording start/stop is handled in the recording page component
  };
  ...
  const startSummaryPolling = React.useCallback((
    ...
    const pollInterval = setInterval(async () => {
      ...
      if (pollCount >= MAX_POLLS) {
        ...
        onUpdate({
          status: 'error',
          error: '요약 생성이 15분 내에 완료되지 않았습니다. 다시 시도하거나 모델 설정을 확인해 주세요.'
        });
        return;
      }
      try {
        ...
        // Tauri invoke() can reject with a plain string instead of an Error.
        // Force string conversion so backend error text is preserved instead of
        // collapsing to "Unknown error".
        const errorText =
