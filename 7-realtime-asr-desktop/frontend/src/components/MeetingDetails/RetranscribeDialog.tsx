...
// Model selection removed — using fixed CNN STT model
...
export function RetranscribeDialog({
  ...
  const availableModels: { key: string; label: string; provider: string; name: string; displayName: string; size_mb: number }[] = [
    { key: 'cnn_stt:cnn-stt-tdt', label: 'CNN STT (Local)', provider: 'cnn-stt', name: 'cnn-stt-tdt', displayName: 'CNN STT', size_mb: 640 }
  ];
  const selectedModelKey = 'cnn_stt:cnn-stt-tdt';
  const setSelectedModelKey = (_k: string) => {};
  ...
  const selectedModelDetails = useMemo((): { key: string; label: string; provider: string; name: string; displayName: string; size_mb: number } | undefined => {
  ...
  const isCnnSttModel = selectedModelDetails?.provider === 'cnn-stt';

  useEffect(() => {
    if (isCnnSttModel && selectedLang !== 'auto') {
      setSelectedLang('auto');
    }
  }, [isCnnSttModel, selectedLang]);
      ...
      const unlistenComplete = await listen<RetranscriptionResult>(
        ...
        async (event) => {
          if (event.payload.meeting_id === meetingId) {
            ...
            setIsProcessing(false);
            toast.success(
              `재전사 완료! 세그먼트 ${event.payload.segments_count}개를 생성했습니다.`
            );
  ...
  const handleStartRetranscription = async () => {
    ...
    try {
      const languageToSend = isCnnSttModel ? null : selectedLang === 'auto' ? null : selectedLang;
      await Analytics.track('enhance_transcript_started', {
        language: isCnnSttModel ? 'auto' : (selectedLang === 'auto' ? 'auto' : selectedLang),
            ...
            !isCnnSttModel ? (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  특정 언어를 선택하면 정확도가 올라가며, 자동 감지를 그대로 사용해도 됩니다
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  CNN STT 모델은 언어 선택을 지원하지 않으며 항상 자동 감지를 사용합니다.
                </p>
              </div>
            )
          )}
