...
export function RecordingSettings({ onSave }: RecordingSettingsProps) {
  ...
  const savePreferences = async (prefs: RecordingPreferences) => {
    ...
    try {
      ...
      const micDevice = prefs.preferred_mic_device || '기본';
      const systemDevice = prefs.preferred_system_device || '기본';
      toast.success("기기 설정을 저장했습니다", {
        description: `마이크: ${micDevice}, 시스템 오디오: ${systemDevice}`
      });
    } catch (error) {
      console.error('Failed to save recording preferences:', error);
      toast.error("기기 설정을 저장하지 못했습니다", {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between p-4 border rounded-lg">
        <div className="flex-1">
          <div className="font-medium">녹음 파일 저장</div>
          <div className="text-sm text-gray-600">
            녹음을 중지하면 오디오 파일을 자동으로 저장합니다
          </div>
        </div>
        <Switch
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 border rounded-lg bg-blue-50">
            ...
            <div className="text-xs text-blue-600 mt-1">
              녹음 파일은 다음과 같이 타임스탬프와 함께 저장됩니다: recording_YYYYMMDD_HHMMSS.{preferences.file_format}
            </div>
          </div>
        </div>
      )}
      ...
      {!preferences.auto_save && (
        <div className="p-4 border rounded-lg bg-yellow-50">
          <div className="text-sm text-yellow-800">
            오디오 저장이 꺼져 있습니다. "녹음 파일 저장"을 켜면 회의 오디오가 자동으로 저장됩니다.
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between p-4 border rounded-lg">
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-600">
            녹음이 시작될 때 참가자에게 안내하도록 알림을 표시합니다
          </div>
        </div>
        <Switch
      ...
      <div className="space-y-4">
        <div className="border-t pt-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            녹음에 사용할 기본 마이크와 시스템 오디오 장치를 설정합니다. 새 녹음을 시작할 때 자동으로 선택됩니다.
          </p>
