...
function categorizeError(error: string): string {
  ...
  if (lowerError.includes('disk') ||
    ...
    return '저장 공간 오류 – 디스크 여유 공간을 확인해 주세요';
  }

  if (lowerError.includes('invalid') || lowerError.includes('validation')) {
    return '파일 검증에 실패했습니다 – 다시 다운로드해 주세요';
  }
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-white rounded-lg shadow-lg border border-gray-200 p-3 relative">
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasError ? (
          <p className="text-xs text-red-600">{download.error || '다운로드 실패'}</p>
        ) : isComplete ? (
          <p className="text-xs text-green-600">다운로드 완료</p>
        ) : isCancelled ? (
          <p className="text-xs text-gray-600">다운로드 취소됨</p>
        ) : (
          <>
            {/* Progress bar */}
...
export function useDownloadProgressToast() {
  ...
  // Listen to CNN STT download events
  useEffect(() => {
    const unlistenProgress = listen<{
      ...
      speed_mbps?: number;
      status?: string;
    }>('cnn-stt-model-download-progress', (event) => {
      const { modelName, progress, downloaded_mb, total_mb, speed_mbps, status } = event.payload;

      const downloadData: DownloadProgress = {
        modelName,
        displayName: '전사 모델 (CNN STT)',
        progress,
    ...
    const unlistenComplete = listen<{ modelName: string }>(
      'cnn-stt-model-download-complete',
      (event) => {
