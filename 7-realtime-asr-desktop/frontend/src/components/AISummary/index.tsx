...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const currentSummary = useMemo(() => {
    if (!summary) {
      return {
        Agenda: { title: "안건", blocks: [] },
        Decisions: { title: "결정 사항", blocks: [] },
        ActionItems: { title: "액션 아이템", blocks: [] },
        ClosingRemarks: { title: "마무리", blocks: [] }
      };
    }
  ...
  const convertToMarkdown = () => {
    let markdown = `# 회의 AI 요약: ${meeting?.id || '알 수 없음'} - ${meeting?.title || '제목 없는 회의'}\n\n`;
    markdown += `## 날짜: ${meeting?.created_at ? new Date(meeting.created_at).toLocaleDateString('ko-KR') : new Date().toLocaleDateString('ko-KR')}\n\n`;

    Object.entries(currentSummary).forEach(([key, section]) => {
      if (key === 'title') {
        markdown = `# ${section.title || 'AI 요약'}\n\n`;
      } else {
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">요약 생성 중 오류</h3>
      </div>
      <p className="text-red-600 text-sm">{error}</p>
      <p className="text-red-500 text-xs mt-2">모델 설정과 API 키를 확인한 뒤 다시 시도해 주세요.</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing' ? '전사 처리 중' : '요약 생성 중'}
          </h3>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">표시할 요약이 없습니다.</p>
        <p className="text-gray-500 text-sm mt-1">새 요약을 생성해 보세요.</p>
      </div>
    );
  }
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span>{selectedBlocks.length > 1 ? `${selectedBlocks.length}개 블록 복사` : '블록 복사'}</span>
          </button>
          <button
          ...
          >
            <span></span>
            <span>{selectedBlocks.length > 1 ? `${selectedBlocks.length}개 블록 삭제` : '블록 삭제'}</span>
          </button>
        </div>
      )}
