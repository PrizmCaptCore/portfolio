        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertDescription className="text-amber-800 mt-2">
            ...
            {!hasMicrophone && (
              <>
                <p className="mb-3">
                  Relay Assistant이 회의를 녹음하려면 마이크 접근 권한이 필요합니다. 사용 가능한 마이크 장치가 감지되지 않았습니다.
                </p>
                <div className="space-y-2 text-sm mb-4">
                  <p className="font-medium">다음을 확인해 주세요:</p>
                  <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>마이크가 연결되어 있고 켜져 있는지</li>
                    <li>시스템 설정에서 마이크 권한을 허용했는지</li>
                    <li>다른 앱이 마이크를 단독으로 사용하고 있지는 않은지</li>
                  </ul>
                </div>
              </>
            )}
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? '시스템 오디오 캡처를 사용할 수 없습니다. 마이크 녹음은 계속 가능하지만 컴퓨터 사운드는 녹음되지 않습니다.'
                    : '시스템 오디오 캡처도 사용할 수 없습니다.'}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">macOS에서 시스템 오디오를 활성화하려면:</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                      <li>BlackHole 2ch 같은 가상 오디오 장치를 설치하세요</li>
                      <li>Relay Assistant에 화면 녹화 권한을 부여하세요</li>
