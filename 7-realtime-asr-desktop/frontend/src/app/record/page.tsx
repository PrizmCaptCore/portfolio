...
import { useTranslationKeepAlive } from '@/hooks/useTranslationKeepAlive';
...
export default function RecordPage() {
  ...
  const { handleRecordingStart, isAutoStarting } = useRecordingStart(
    isRecording,
    setIsRecordingState,
    showModal
  );
  ...
  // Keep Modal translate warm for the whole recording session so toggling
  // translation back on mid-meeting doesn't eat a cold-start. See hook body.
  useTranslationKeepAlive();
  ...
  const handleRecovery = async (meetingId: string) => {
    try {
      ...
      if (result.success) {
        toast.success('회의를 복구했습니다!', {
          description: result.audioRecoveryStatus?.status === 'success'
            ? '전사와 오디오를 복구했습니다'
            : '전사를 복구했습니다 (오디오는 복구되지 않음)',
      ...
      // Don't re-throw — toast already shows the error; re-throwing would trigger
      // a redundant browser alert() in TranscriptRecovery component.
    }
  };
  ...
  const showRecordingControlsChrome =
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Recording controls — hidden during sidebar/tray auto-start until backend is recording */}
        {showRecordingControlsChrome && (
            <div className="fixed bottom-12 left-0 right-[var(--docked-ai-rail-width,0px)] z-10">
              <div
        ...
        {/* Auto-start from sidebar/tray: indeterminate bar until recording is live */}
        {isAutoStarting && (
          <div className="fixed bottom-12 left-0 right-[var(--docked-ai-rail-width,0px)] z-10">
            <div
              className="flex justify-center pl-8 transition-[margin] duration-300"
            ...
            >
              <div className="w-full max-w-xs rounded-xl border border-gray-200 bg-white px-4 py-2.5 shadow-md">
                <p className="mb-2 text-center text-xs font-medium text-gray-700">
                  녹음 준비 중…
                </p>
                <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
                  <div className="h-full w-[38%] rounded-full bg-blue-600 animate-recording-prepare-bar" />
                </div>
              </div>
            </div>
          </div>
        ...
        <StatusOverlays
          isStarting={
            status === RecordingStatus.STARTING && !isAutoStarting
          }
