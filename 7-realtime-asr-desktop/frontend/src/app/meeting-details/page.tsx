...
function MeetingDetailsContent() {
  ...
  const setupAutoGeneration = useCallback(async () => {
    ...
    // Summary always routes through the Relay gateway (resolved server-side
    // in llm_client.rs), so there's no per-user model config to check. Fresh
    // installs seed a placeholder row on first launch (setup.rs), but we
    // don't gate on it — auto-gen should run whenever the user navigated
    // from a recording and the auto-summary toggle is on.
  ...
  if ((isLoading || isLoadingTranscripts) || !meetingDetails) {
    return <div className="flex h-full min-h-0 items-center justify-center">
