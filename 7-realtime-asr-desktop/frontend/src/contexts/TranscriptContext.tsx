...
interface TranscriptContextType {
  ...
  patchTranscriptBySequenceId: (sequenceId: number, patch: Partial<Transcript>) => void;
  copyTranscript: () => void;
  flushBuffer: () => void;
...
/** Merge two updates for the same sequence_id; keep stable `id` for streaming UI. */
function mergeLiveTranscriptSegments(existing: Transcript, incoming: Transcript): Transcript {
  // Final updates always win. Partial updates may replace partial rows, but never overwrite an established final row.
  const isFinalIncoming = incoming.is_partial === false;
  const isFinalExisting = existing.is_partial === false;
  const useIncoming = isFinalIncoming || (!isFinalExisting && existing.is_partial === true);

  // Once a final row is established, preserve its metadata fields. A previous
  // spread-based merge let late partial windows overwrite final timing data.
  const base = isFinalExisting ? existing : incoming;

  return {
    ...base,
    id: existing.id,
    sequence_id: existing.sequence_id ?? incoming.sequence_id,
    text: useIncoming ? incoming.text : existing.text,
    is_partial: isFinalIncoming ? false : (isFinalExisting ? false : incoming.is_partial),
    // Stable-prefix commit tail: finals always clear it; partial-over-partial uses
    // the latest incoming tail. If a final has already locked the row, keep it cleared.
    tentative_text: isFinalIncoming || isFinalExisting ? undefined : incoming.tentative_text,
    // Translation fields are patched separately, so prefer whichever side already has data.
    translated_text: existing.translated_text ?? incoming.translated_text,
    translation_pending: existing.translation_pending ?? incoming.translation_pending,
    translation_error: existing.translation_error ?? incoming.translation_error,
    coaching_intent: existing.coaching_intent ?? incoming.coaching_intent,
    coaching_replies: existing.coaching_replies ?? incoming.coaching_replies,
  };
}
...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  // Note: auto-scroll-to-bottom on new transcripts USED to live here as a
  // duplicate mechanism (10px-tolerance "is user at bottom" check + smooth
  // scrollTo on every transcripts change). It competed with the proper
  // useAutoScroll hook in VirtualizedTranscriptView and overrode every
  // user-scrolled-up gate the hook tried to enforce — partial transcript
  // updates fire on essentially every keystroke, so even a 5px scroll
  // upward was within the 10px tolerance and would be yanked back down on
  // the very next partial. useAutoScroll is now the single source of
  // truth; it tracks scroll *direction* (not just bottom-distance), kills
  // auto-scroll on the first upward delta, and re-enables only on
  // returning to the bottom. No replacement effect is needed here.
  ...
  useEffect(() => {
    ...
    let firstSequenceSeen = -1; // Track first sequence_id to handle non-zero starts
        ...
        setTranscripts(prev => {
          ...
          next.forEach((t, i) => {
            if (t.sequence_id !== undefined) {
              indexBySeq.set(t.sequence_id, i);
            }
          });
          ...
          for (const incoming of allNewTranscripts) {
            if (incoming.sequence_id === undefined) {
              next.push(incoming);
              addedCount++;
              continue;
            }
            const idx = indexBySeq.get(incoming.sequence_id);
            if (idx === undefined) {
              indexBySeq.set(incoming.sequence_id, next.length);
              next.push(incoming);
              addedCount++;
            } else {
              next[idx] = mergeLiveTranscriptSegments(next[idx], incoming);
              mergedCount++;
            }
          }
          ...
          console.log(
            `Transcripts: +${addedCount} new, ${mergedCount} merged (batch ${allNewTranscripts.length})`
          );
    ...
    const setupListener = async () => {
      try {
        ...
        unlistenFn = await transcriptService.onTranscriptUpdate((update) => {
          ...
          // Same sequence_id may arrive again with richer text (e.g. refined partial); overwrite and keep one stable row id.
          const priorBuffered = transcriptBuffer.get(update.sequence_id);
          const rowId = priorBuffered
            ? priorBuffered.id
          ...
          const newTranscript: Transcript = {
            ...
            // Stable-prefix commit: dimmed tail for partials, undefined for finals.
            tentative_text: update.is_partial ? update.tentative_text : undefined,
          };

          // Add to buffer
          transcriptBuffer.set(update.sequence_id, newTranscript);

          // Initialize lastProcessedSequence on first transcript so sequential
          // processing starts from the correct base (sequence_id may not start at 0)
          if (firstSequenceSeen === -1) {
            firstSequenceSeen = update.sequence_id;
            lastProcessedSequence = update.sequence_id - 1;
          }
  ...
  const patchTranscriptBySequenceId = useCallback((sequenceId: number, patch: Partial<Transcript>) => {
    setTranscripts(prev =>
      prev.map(t => (t.sequence_id === sequenceId ? { ...t, ...patch } : t))
    );
  }, []);
