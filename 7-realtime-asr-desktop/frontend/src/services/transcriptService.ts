...
export class TranscriptService {
  ...
  /**
   * Listen for CNN STT model download complete event
   * @param callback - Function to call when CNN STT model download completes
   * @returns Promise that resolves to unlisten function
   */
  async onCnnSttModelDownloadComplete(callback: (modelName: string) => void): Promise<UnlistenFn> {
    return listen<ModelDownloadCompletePayload>('cnn-stt-model-download-complete', (event) => {
      callback(event.payload.modelName);
    });
  }
}
