      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        try {
          const responseData = await storageService.saveMeeting(
            savedMeetingName || meetingTitle || '새 회의',  // PREFER savedMeetingName (backend source)
            freshTranscripts,
            folderPath
          );
          ...
          try {
            ...
            setCurrentMeeting({ id: meetingId, title: savedMeetingName || meetingTitle || '새 회의' });
          }
          ...
          toast.success('녹음이 저장되었습니다!', {
            description: `전사 세그먼트 ${freshTranscripts.length}개를 저장했습니다.`,
            action: {
              label: '회의 보기',
              onClick: () => {
          ...
          toast.error('회의 저장에 실패했습니다', {
            description: saveError instanceof Error ? saveError.message : '알 수 없는 오류'
          });
          throw saveError;
        }
      } else {
