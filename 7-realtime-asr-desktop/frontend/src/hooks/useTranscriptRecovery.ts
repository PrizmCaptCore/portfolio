...
export function useTranscriptRecovery(): UseTranscriptRecoveryReturn {
  ...
  const loadMeetingTranscripts = useCallback(async (meetingId: string): Promise<StoredTranscript[]> => {
    try {
      ...
      // StoredTranscripts are spread from TranscriptUpdate, so the actual field is
      // sequence_id (snake_case), not sequenceId. Handle both for safety.
      transcripts.sort((a, b) => {
        const seqA = (a as any).sequence_id ?? a.sequenceId ?? 0;
        const seqB = (b as any).sequence_id ?? b.sequenceId ?? 0;
        return seqA - seqB;
      });
      return transcripts;
    } catch (error) {
  ...
  const recoverMeeting = useCallback(async (meetingId: string): Promise<{ success: boolean; audioRecoveryStatus?: AudioRecoveryStatus | null; meetingId?: string }> => {
    ...
    try {
      ...
      // Note: StoredTranscripts are spread from TranscriptUpdate, so numeric fields
      // use snake_case (sequence_id, audio_start_time, etc.) not camelCase.
      const formattedTranscripts = transcripts.map((t, index) => ({
        id: t.id?.toString() || `${Date.now()}-${index}`,
        text: t.text,
        // timestamp is required by Rust's TranscriptSegment — fall back to ISO string
        timestamp: t.timestamp || new Date().toISOString(),
        sequence_id: (t as any).sequence_id ?? t.sequenceId ?? index,
        audio_start_time: (t as any).audio_start_time ?? undefined,
        audio_end_time: (t as any).audio_end_time ?? undefined,
        duration: (t as any).duration ?? undefined,
      }));
