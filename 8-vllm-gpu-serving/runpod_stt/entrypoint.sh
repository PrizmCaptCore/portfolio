...
# Loops on exit so the in-process /admin/reload handler can sys.exit(0)
# and the next iteration picks up freshly-pulled code without a full
# image rebuild. Heavy deps (PyTorch, NeMo, model weights) are baked
# into the image once; code-only updates skip the build entirely.
#
# Required env:
...
# (COPY'd at image build time) is used. The Pod keeps serving — it just
# falls back to whatever code was baked.
...
SRC=$REPO_DIR/runpod_stt/server.py
...
pull_code() {
  ...
  if [ -d "$REPO_DIR/.git" ]; then
    ...
    git -C "$REPO_DIR" reset --hard FETCH_HEAD --quiet || return 0
  else
  ...
  # Syntax-check before swap so a broken push doesn't put the Pod into
  # an infinite restart loop. If the new file fails parsing, leave the
  # previous (working) $DST in place.
  if ! python -c "import ast, sys; ast.parse(open('$SRC').read())" 2>/dev/null; then
...
while true; do
  pull_code
  python "$DST" --host 0.0.0.0 --port 8765
  ec=$?
