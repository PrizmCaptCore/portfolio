"""
Stress test for the Embedding (Qwen3-Embedding-0.6B) service via the
production example.com gateway.

Login → access JWT → N concurrent POSTs to /ai/embed/, released
...
emit a vector-count / dim sample so a 200 OK with an empty body
still shows up as a failure in the breakdown.
...
def _build_body() -> dict:
    return {
        "texts": [
            "회의 요약: STT race 픽스를 이번 주에 출시.",
            "다음 스프린트는 임베딩 파이프라인에 집중.",
        ],
    }
...
def _parse(data: dict) -> str:
    ...
    if not vectors:
        return ""
    return f"{len(vectors)} vectors, dim={len(vectors[0])}"
...
async def main_async(args):
    ...
    async with httpx.AsyncClient(verify=True) as http:
        ...
        print(f"Service: embedding  path: {PATH}  timeout: {args.timeout:.0f}s",
              flush=True)
        ...
        await asyncio.gather(
            *(
                one_request(i, http, base, jwt, barrier, args.timeout, results)
