"""
Stress test for the Translate (TranslateGemma) service via the
production example.com gateway.

Login → access JWT → N concurrent POSTs to /ai/translate/, released
...
def _build_body() -> dict:
    return {
        "text":
            "Hello team. We agreed to ship the STT lock fix this week, "
            "and next sprint focuses on the embedding pipeline.",
    }


def _parse(data: dict) -> str:
    return (data.get("translation") or "")[:80]
...
async def main_async(args):
    ...
    async with httpx.AsyncClient(verify=True) as http:
        ...
        print(f"Service: translate  path: {PATH}  timeout: {args.timeout:.0f}s",
              flush=True)
