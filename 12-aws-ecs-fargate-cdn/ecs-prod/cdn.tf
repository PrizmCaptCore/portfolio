# ==================== CloudFront + S3 SPA (prod) ====================
# relay.example.com / www.relay.example.com 진입점을 CloudFront로 통일:
#   - default behavior → S3 (SPA: frontend/dist)
#   - /api/*, /admin/*, /static/*, /media/*, /accounts/*, /health-check/* → prod ALB
...
# ==================== ACM (us-east-1) ====================
# DNS는 external DNS registrar 관리 → 검증용 CNAME을 external DNS registrar에 수동 추가 필요

resource "aws_acm_certificate" "cloudfront" {
  provider                  = aws.us_east_1
  domain_name               = var.acm_domain       # relay.example.com
  subject_alternative_names = [var.acm_domain_www] # www.relay.example.com
...
# serving the SPA HTML to API clients.
...
resource "aws_cloudfront_distribution" "spa" {
  ...
  aliases             = [var.acm_domain, var.acm_domain_www]
  ...
  # `alb-api` to be replaced with the SPA HTML, which breaks API clients.
...
# apply 후 `terraform output` 으로:
#   1) acm_validation_records → external DNS registrar에 CNAME 수동 추가 (검증용)
#   2) cloudfront_domain_name → external DNS registrar에서
#        www  CNAME  <cloudfront_domain_name>
#      apex relay.example.com은 external DNS registrar가 CNAME/ALIAS 미지원 → external DNS registrar forwarding(301→www) 유지
